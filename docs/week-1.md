...
- [x] **T1-013**: Create `SequenceConfig.as` *(already exists)*
...
- [ ] **T1-039**: Implement `CastlePart` model:
  ```actionscript
  class CastlePart {
    id: String,
  ...
  ```
- [ ] **T1-040**: Create `CastleState` snapshot class
- [ ] **T1-041**: Define castle component taxonomy:
  - Foundation: 3 tiers
  - Walls: 4 variants
  - Towers: 4 types
  - Decorations: 5+ types
...
- [ ] **T1-042**: Implement `CastleArchitect`:
  ```actionscript
  ...
  ```
- [ ] **T1-043**: Design castle growth algorithm:
...
- [ ] **T1-044**: Implement grid-based placement
- [ ] **T1-045**: Castle serialization/deserialization

### 2.3 Castle Mechanics
- [ ] **T1-046**: Implement castle health system
- [ ] **T1-047**: Create damage calculation logic
- [ ] **T1-048**: Implement repair mechanics
- [ ] **T1-049**: Add upgrade path logic

### 2.4 Game Metrics
- [ ] **T1-050**: Track comprehensive metrics:
  ```actionscript
  class GameMetrics {
    totalTrials: int,
    correctTrials: int,
  ...
  ```
- [ ] **T1-051**: Debug metrics display
- [ ] **T1-052**: Metrics persistence integration

### 2.5 Effects Logic
- [ ] **T1-053**: Particle system data:
  - Confetti parameters
  - Sparkle parameters
  - Dust parameters
- [ ] **T1-054**: Achievement notification logic
- [ ] **T1-055**: Floating score popup logic

### 2.6 Error Handling & Testing
- [ ] **T1-056**: Castle system error handling
- [ ] **T1-057**: Performance profiling (60 FPS)
- [ ] **T1-058**: Architecture documentation
...
- [ ] **T1-059**: Create `LayoutManager.as` multi-resolution support
- [ ] **T1-060**: Design responsive grid system
- [ ] **T1-061**: Safe zone handling (notch, etc.)

### 3.2 Stimulus Presentation
- [ ] **T1-062**: Design visual vocabulary:
  - 6 shapes: Circle, Square, Triangle, Star, Hexagon, Diamond
  - 8 colors (color-blind safe)
  - Number mode: 1-9
  - Letter mode: A-F
- [ ] **T1-063**: Implement `StimulusView`:
  - Smooth fade-in/fade-out
  - Scale animations
  - Glow effects
- [ ] **T1-064**: Create `StimulusQueue` for sequential display
- [ ] **T1-065**: Add progress indicator during stimulus
- [ ] **T1-066**: Implement "focus mode" (dim background)

### 3.3 Input UI
- [ ] **T1-067**: Button states: Normal, Hover, Pressed, Disabled, Correct, Wrong
- [ ] **T1-068**: Visual indicators per game state
- [ ] **T1-069**: Haptic feedback integration (mobile)
- [ ] **T1-070**: Input feedback animations
...
- [ ] **T1-076**: Complete HUD:
  ...
  - Castle progress bar
  - Mini castle preview
- [ ] **T1-077**: State indicator panel (Ready/Watch/Answer/Result)

### 3.6 Menu System
- [ ] **T1-078**: Main Menu:
  - Play / Continue
  - Mode Select (Forward, Reverse, Sort)
...
### 3.8 Visual Effects
- [ ] **T1-083**: Screen transitions (fade, slide, zoom)
- [ ] **T1-084**: Particle rendering:
  - Confetti (correct)
  - Sparkles (streak)
  - Dust (construction)
- [ ] **T1-085**: Screen shake (wrong answer)
